...
Close PowerShell, open a **new** one. Then install **pnpm via corepack** (it ships with
Node) — do **not** run `npm install -g pnpm`, because that pulls the newest pnpm, which
is often **too new for your Node and crashes on every run** (a common cause of the web
server never starting). Corepack installs the exact version this project is pinned to:

```powershell
node --version          # must be v20 or higher — if it's older, reinstall Node LTS first
corepack enable
